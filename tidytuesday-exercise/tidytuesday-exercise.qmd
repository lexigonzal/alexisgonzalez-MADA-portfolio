...
```
####The data is not very linear but the RMSE tells me that it is not terrible. The r-squared is low so this model is not capturing the pattern well. I will now try a random forest model to better capture the pattern.
